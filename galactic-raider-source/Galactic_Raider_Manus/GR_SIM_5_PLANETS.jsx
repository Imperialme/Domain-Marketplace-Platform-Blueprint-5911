...
const PLANETS={
  Earth:{id:"earth",name:"Earth",ico:"🌍",currency:"USD",rate:1.0,gdp:2.5,desc:"Base economy. All other planets reference Earth.",color:"#2E7D32",bg:"#E8F5E9",companies:[
    {t:"SLKT",n:"Silk Road Tech",s:"Technology",price:348.94,pe:18.4,div:.8,b:1.8},
    {t:"TNPT",n:"Titan Petroleum",s:"Energy",price:351.54,pe:11.3,div:1.8,b:1.2},
    {t:"MDCR",n:"MediCore Group",s:"Healthcare",price:198.40,pe:22.1,div:1.2,b:.8},
    {t:"FRMN",n:"Frontier Mining",s:"Mining",price:15.80,pe:8.5,div:.5,b:1.6},
  ]},
  Mars:{id:"mars",name:"Mars",ico:"🔴",currency:"MCR",rate:.85,gdp:3.8,desc:"Mining economy. Lithium, iron ore, rare earths. Volatile. Contagion from Earth in 2 turns.",color:"#C62828",bg:"#FFEBEE",contagionDelay:2,contagionFactor:.5,companies:[
    {t:"MXMN",n:"Mars Extraction Co",s:"Mining",price:42.50,pe:9.2,div:.3,b:2.1,desc:"Largest lithium and rare earth extractor on Mars. Supplies 30% of Earth EV battery materials."},
    {t:"RDST",n:"RedDust Energy",s:"Energy",price:18.20,pe:7.8,div:.8,b:1.8,desc:"Solar and nuclear energy for Mars colony. Regulated utility. Stable."},
    {t:"MROBOT",n:"Mars Robotics Corp",s:"Technology",price:95.30,pe:22.4,div:.1,b:2.5,desc:"Autonomous mining robots. High margin. Weather risk from dust storms."},
    {t:"MFOOD",n:"HydroFarm Mars",s:"Agriculture",price:28.60,pe:14.1,div:1.2,b:1.1,desc:"Indoor hydroponic food production for 50,000 colonists. Essential service."},
  ]},
  Venus:{id:"venus",name:"Venus",ico:"🟡",currency:"VCR",rate:.75,gdp:2.1,desc:"Energy export economy. Extreme heat. Fully automated. High margins. Contagion from Earth in 2 turns.",color:"#F57F17",bg:"#FFF8E1",contagionDelay:2,contagionFactor:.67,companies:[
    {t:"VSOL",n:"Venus Solar Array",s:"Energy",price:156.40,pe:16.8,div:2.4,b:.9,desc:"Largest solar energy harvester in the solar system. 99.8% uptime. Exports to Earth."},
    {t:"VATM",n:"AtmoRefine Venus",s:"Manufacturing",price:67.20,pe:13.5,div:1.0,b:1.3,desc:"Atmospheric chemical processing. Sulfuric acid, carbon compounds for industrial use."},
    {t:"VROB",n:"Venus Autoworks",s:"Manufacturing",price:38.90,pe:11.2,div:.6,b:1.6,desc:"Fully automated manufacturing. No human workers. 45% net margins."},
  ]},
  Jupiter:{id:"jupiter",name:"Jupiter",ico:"🟠",currency:"JCR",rate:.90,gdp:4.2,desc:"Robotic economy. Highest margins in solar system. Storm events every 50-100 turns cause massive disruption. Buy post-storm. Contagion from Earth in 3 turns.",color:"#E65100",bg:"#FFF3E0",contagionDelay:3,contagionFactor:.83,stormRisk:true,companies:[
    {t:"JATM",n:"Jupiter AutoMine",s:"Mining",price:212.30,pe:18.4,div:.2,b:2.8,desc:"Deep atmosphere mining robots. Rare materials including Ryzolith. 52% net margin. Storm risk."},
    {t:"JRES",n:"JupiterResearch AI",s:"Technology",price:445.60,pe:28.2,div:.1,b:3.2,desc:"AI research complex. Fully automated. Develops tech licensed to all planets."},
    {t:"JFUS",n:"Fusion Power Jupiter",s:"Energy",price:88.40,pe:14.6,div:.8,b:1.9,desc:"Experimental fusion reactors. High risk, high reward. Could power the entire solar system."},
  ]},
};
  ...
  const advance=useCallback(()=>{
    ...
    ALL_PLANETS.forEach(pd=>{
      ...
      p.cos=p.cos.map(c=>{
        ...
        const move=1+(Math.random()-.5)*.10*c.b*(stormMult*.3+.7);
        const macroEffect=1+(p.gdp/100*.3*c.b);
        let np=Math.max(.1,Math.round(c.price*move*macroEffect*100)/100);
        // Post-storm recovery bounce
        if(pd.stormRisk&&p.stormTurn>0&&!p.stormActive&&s.turn-p.stormTurn<10)np=Math.round(np*1.03*100)/100;
    ...
    <div style={{padding:14,display:"flex",flexDirection:"column",gap:12}}>
      ...
      {tab==="transfer"&&<>
        <div style={{background:"#fff",borderRadius:13,padding:13,border:"1px solid #e8ebe8"}}>
          ...
          {[["Earth","Mars",50000],["Earth","Venus",50000],["Earth","Jupiter",50000],["Mars","Earth",5000],["Venus","Earth",5000],["Jupiter","Earth",10000]].filter(([from])=>(d.wallets?.[from]||0)>0).map(([from,to,amt])=>{
            const fpd=ALL_PLANETS.find(p=>p.name===from),tpd=ALL_PLANETS.find(p=>p.name===to);
            const usd=Math.round(amt*fpd.rate*100)/100;const fee=Math.round(usd*.005*100)/100;const recv=Math.round((usd-fee)/tpd.rate*100)/100;
            return <div key={from+to} style={{background:"#f8fbf8",borderRadius:9,padding:"10px 12px",marginBottom:7,border:"1px solid #e8ebe8"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}><div style={{fontSize:12,fontWeight:700,color:DK}}>{fpd.ico} {from} → {tpd.ico} {to}</div><div style={{fontSize:11,color:"#aaa"}}>Fee: {fm(fee)}</div></div>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:11,color:"#666"}}>Send {fm(amt)} {fpd.currency}</span><span style={{fontSize:11,fontWeight:700,color:G}}>Receive {fm(recv)} {tpd.currency}</span></div>
              <button onClick={()=>doTransfer(from,to,amt)} disabled={(d.wallets?.[from]||0)<amt} style={{width:"100%",background:(d.wallets?.[from]||0)>=amt?fpd.color:"#e0e0e0",color:"#fff",border:"none",borderRadius:7,padding:"8px 0",fontWeight:700,fontSize:11,cursor:(d.wallets?.[from]||0)>=amt?"pointer":"not-allowed"}}>Transfer {fm(amt)} {fpd.currency}</button>
            </div>;})}
        </div>
