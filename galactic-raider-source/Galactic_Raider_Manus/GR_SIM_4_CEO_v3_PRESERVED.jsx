...
export default function CEOSim(){
  ...
  return <div style={{maxWidth:430,margin:"0 auto",background:"#F0F4F0",minHeight:"100vh",fontFamily:"system-ui,sans-serif"}}>
    ...
    <div style={{padding:14,display:"flex",flexDirection:"column",gap:12}}>
      {/* DECISIONS TAB */}
      {tab==="decisions"&&<>
        {pendingDecs.length===0&&<div style={{background:"#fff",borderRadius:14,padding:20,border:"1px solid #e8ebe8",textAlign:"center"}}>
          <div style={{fontSize:40,marginBottom:10}}>✅</div>
          <div style={{fontSize:15,fontWeight:700,color:DK,marginBottom:6}}>No Pending Decisions</div>
          <div style={{fontSize:12,color:"#888",lineHeight:1.6}}>CEO decisions fire every 30 turns for companies where you own 10%+. Advance turns to see new decisions, or buy shares to get board access.</div>
          <div style={{marginTop:12,padding:"10px 14px",background:"#f8fbf8",borderRadius:9,fontSize:11,color:"#555",lineHeight:1.6}}>4 pre-loaded decisions were available when you started. Resolve them from the resolved section below, or advance to turn 30 to trigger new ones for companies you own.</div>
        </div>}
        {pendingDecs.map(dec=>{
        ...
        {d.cos.map(c=>{
          ...
          return <div key={c.t} style={{background:"#fff",borderRadius:13,padding:13,border:"1px solid #e8ebe8"}}>
            ...
            <div style={{marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11,color:"#888"}}>Your ownership</span><span style={{fontSize:12,fontWeight:700,color:tierColor}}>{pct.toFixed(2)}%{held>0?" ("+held.toLocaleString()+" shares)":""}</span></div>
              <div style={{height:6,background:"#f0f0f0",borderRadius:3,overflow:"hidden"}}>
                <div style={{height:"100%",background:"linear-gradient(90deg,"+G+","+BL+")",borderRadius:3,width:Math.min(100,pct)+"%",transition:"width .3s"}}/>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}><span style={{fontSize:8,color:"#ccc"}}>0%</span><span style={{fontSize:8,color:G}}>10% Board</span><span style={{fontSize:8,color:BL}}>25% Strategy</span><span style={{fontSize:8,color:PU}}>50% Control</span></div>
            </div>
            {/* Buy buttons */}
            <div style={{display:"flex",gap:6}}>
      ...
      {tab==="log"&&<div style={{background:"#fff",borderRadius:14,padding:13,border:"1px solid #e8ebe8"}}>
        ...
        <div style={{fontSize:11,color:"#888",marginBottom:12}}>Quarterly logging — shows board decisions, CEO reputation changes, auto-resolved events.</div>
