...
export default function RedemptionSim(){
  ...
  if(!S.current){S.current={
    ...
    debtHistory:[],
    log:[
      {turn:1,type:"SYSTEM",msg:"Redemption System online. Earn points through philanthropy → spin tokens → Wheel of Fortune debt relief. Current debt: $250K.",good:true},
    ...
    const animate=(ts)=>{
      ...
      if(progress<1){requestAnimationFrame(animate);}
        ...
        if(result.outcome.startsWith("debt")&&result.pct>0){
          ...
          s2.log.unshift({turn:s2.turn,type:"SPIN",msg:"WHEEL RESULT: "+result.pct+"% debt relief — "+fm(relief)+" forgiven! Debt now: "+fm(s2.totalDebt),good:true});
          ...
          s2.log.unshift({turn:s2.turn,type:"SPIN",msg:"WHEEL RESULT: Consolation prize — +"+result.bonus+" redemption points. Total: "+s2.redeemPts,good:true});
    ...
    <div style={{padding:14,display:"flex",flexDirection:"column",gap:12}}>
      ...
      {tab==="wheel"&&<>
        <div style={{background:"linear-gradient(135deg,#1A0533,#2D1054)",borderRadius:16,padding:16,border:"2px solid rgba(159,122,234,.3)",textAlign:"center"}}>
          <div style={{fontSize:16,fontWeight:800,color:"#E9D5FF",marginBottom:4}}>🎡 Wheel of Fortune</div>
