  ...
  return <div style={{maxWidth:430,margin:"0 auto",background:"#F0F4F0",minHeight:"100vh",fontFamily:"system-ui,sans-serif"}}>
    ...
    <div style={{padding:14,display:"flex",flexDirection:"column",gap:10}}>
      ...
      {tab==="port"&&<>
        ...
        {d.ipos.filter(ip=>ip.allocated>0).length>0&&<div style={{background:"#fff",borderRadius:13,border:"1px solid #e8ebe8",overflow:"hidden"}}>
          <div style={{padding:"11px 14px",fontSize:13,fontWeight:700,color:DK}}>📋 IPO Bookings</div>
          {d.ipos.filter(ip=>ip.allocated>0).map(ip=>{
            const mid=(ip.priceRange[0]+ip.priceRange[1])/2;
            const turnsTo=ip.opens-d.turn;
            return <div key={ip.id} style={{padding:"11px 14px",borderTop:"1px solid #f5f5f5"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div><div style={{fontSize:13,fontWeight:700,color:DK}}>{ip.n}</div><div style={{display:"flex",gap:5,marginTop:3}}><span style={{background:"#f0f4f0",color:"#555",padding:"1px 7px",borderRadius:10,fontSize:9,fontWeight:700}}>{ip.sector}</span><span style={{background:"#EDE7F6",color:PU,padding:"1px 7px",borderRadius:10,fontSize:9,fontWeight:700}}>{ip.planet}</span></div></div>
                <div>{ip.listed?<span style={{background:"#E8F5E9",color:G,padding:"4px 10px",borderRadius:8,fontSize:11,fontWeight:800}}>✅ Listed @ {fm(ip.listPrice)}</span>:<span style={{background:turnsTo<=10?"#FFF8E1":"#E3F2FD",color:turnsTo<=10?AU:BL,padding:"4px 10px",borderRadius:8,fontSize:11,fontWeight:800}}>{turnsTo<=0?"OPEN":"T"+ip.opens+" ("+turnsTo+"t)"}</span>}</div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>
                <div style={{background:"#f8fbf8",borderRadius:7,padding:"7px 6px",textAlign:"center"}}><div style={{fontSize:9,color:"#aaa",marginBottom:1}}>Shares Booked</div><div style={{fontSize:11,fontWeight:700,color:DK}}>{(ip.allocated||0).toLocaleString()}</div></div>
                <div style={{background:"#f8fbf8",borderRadius:7,padding:"7px 6px",textAlign:"center"}}><div style={{fontSize:9,color:"#aaa",marginBottom:1}}>Est. Value</div><div style={{fontSize:11,fontWeight:700,color:DK}}>{fm((ip.allocated||0)*mid)}</div></div>
                <div style={{background:ip.listed?"#E8F5E9":"#f8fbf8",borderRadius:7,padding:"7px 6px",textAlign:"center"}}><div style={{fontSize:9,color:"#aaa",marginBottom:1}}>{ip.listed?"Received":"Awaiting"}</div><div style={{fontSize:11,fontWeight:700,color:ip.listed?G:AU}}>{ip.listed?fm((ip.allocated||0)*ip.listPrice):"At T"+ip.opens}</div></div>
              </div>
              {ip.listed&&ip.currentPrice&&<div style={{marginTop:6,fontSize:11,color:ip.currentPrice>=ip.listPrice?G:R,fontWeight:600}}>Current price: {fm(ip.currentPrice)} · {ip.currentPrice>=ip.listPrice?"+"+fm(ip.currentPrice-ip.listPrice)+" above IPO":fm(ip.listPrice-ip.currentPrice)+" below IPO"}</div>}
            </div>;})}
        </div>}
