...
const PLANETS={
  ...
  Saturn:{id:"saturn",name:"Saturn",ico:"🪐",currency:"STC",rate:.70,gdp:3.1,desc:"Ring mining economy. Ryzolith ore — rarest material in the solar system. Ice water exports. Contagion from Earth in 4 turns.",color:"#7B1FA2",bg:"#F3E5F5",contagionDelay:4,contagionFactor:.45,companies:[
    {t:"SRYZ",n:"Ryzolith Corp",s:"Mining",ip:380.20,price:380.20,pe:12.4,div:.4,b:2.2,desc:"Sole extractor of Ryzolith — the most valuable ore in the solar system. Used in quantum computing. Monopoly position."},
    {t:"SICE",n:"Saturn Ice Export",s:"Agriculture",ip:22.40,price:22.40,pe:9.8,div:1.8,b:.9,desc:"Harvests ring ice, purifies and exports as drinking water to inner planets. Steady demand."},
    {t:"SRNG",n:"Ring Dynamics",s:"Manufacturing",ip:145.60,price:145.60,pe:16.2,div:.5,b:1.7,desc:"Automated ring mining equipment. Supplies tools to all outer planet extractors."},
  ]},
  Mercury:{id:"mercury",name:"Mercury",ico:"☿",currency:"MRC",rate:.60,gdp:5.2,desc:"Solar energy economy. Closest to the Sun — highest solar density in system. Day 430°C, night −180°C. Fully automated. Contagion from Earth in 2 turns.",color:"#455A64",bg:"#ECEFF1",contagionDelay:2,contagionFactor:.55,companies:[
    {t:"MSOL",n:"Mercury Solar Prime",s:"Energy",ip:290.80,price:290.80,pe:20.4,div:2.2,b:1.1,desc:"Largest solar array in the solar system. 24× Earth solar intensity. Exports energy to Earth, Venus, Mars."},
    {t:"MTHM",n:"ThermoMine Mercury",s:"Mining",ip:48.30,price:48.30,pe:10.1,div:.7,b:1.8,desc:"Extracts rare minerals deposited by solar winds. Unique supply of solar-fused elements."},
    {t:"MRBT",n:"Mercury Robotics",s:"Technology",ip:124.50,price:124.50,pe:17.8,div:.3,b:2.0,desc:"Builds extreme-temperature robots for Mercury surface operations. Only company operating at 430°C."},
  ]},
  Uranus:{id:"uranus",name:"Uranus",ico:"🔵",currency:"URU",rate:.55,gdp:1.8,desc:"Ice mining economy. −224°C coldest planet. 42-year seasons mean multi-decade investment cycles. Low volatility, very long-term returns. Contagion from Earth in 5 turns.",color:"#0277BD",bg:"#E1F5FE",contagionDelay:5,contagionFactor:.35,companies:[
    {t:"UICE",n:"Uranus Ice Dynamics",s:"Mining",ip:65.40,price:65.40,pe:11.8,div:2.8,b:.7,desc:"Methane and ammonia ice extraction. Feeds Jupiter fusion reactors. 42-year seasonal cycles."},
    {t:"UCRY",n:"CryoTech Uranus",s:"Technology",ip:185.20,price:185.20,pe:19.6,div:.4,b:1.2,desc:"Cryogenic technology for deep space. Coldest operating environment in the system. High barriers to entry."},
    {t:"URES",n:"Uranus Research Base",s:"Healthcare",ip:42.10,price:42.10,pe:14.2,div:1.1,b:.8,desc:"Extreme-cold biology research. Develops cryogenic medicines. Unique research moat."},
  ]},
  Neptune:{id:"neptune",name:"Neptune",ico:"💜",currency:"NPT",rate:.50,gdp:2.4,desc:"Deep research economy. 2,100 km/h winds. Furthest colony — 4.5 billion km from Sun. Highest risk, highest potential. Contagion from Earth in 6 turns.",color:"#4527A0",bg:"#EDE7F6",contagionDelay:6,contagionFactor:.28,companies:[
    {t:"NRES",n:"Neptune Deep Research",s:"Technology",ip:520.40,price:520.40,pe:32.4,div:.05,b:3.5,desc:"Cutting-edge physics research. Develops technologies 50 years ahead of Earth. Extreme long-term play."},
    {t:"NWIN",n:"Wind Energy Neptune",s:"Energy",ip:38.60,price:38.60,pe:8.8,div:3.2,b:1.4,desc:"Harnesses Neptune's 2,100 km/h winds. Technically challenging but enormous energy output."},
    {t:"NMIN",n:"Neptune Minerals",s:"Mining",ip:156.90,price:156.90,pe:14.1,div:.9,b:2.1,desc:"Extracts unique deep-space minerals found only at Neptune orbit. Quantum material applications."},
  ]},
};
const ALL_PLANETS=Object.values(PLANETS);
// ── PLANET EVENTS — each planet has its own unique event ────
const PLANET_EVENTS={
  Earth:{id:"tax_cut",n:"Tax Policy Change",ico:"🔔",prob:.03,gdpBoost:1.5,dur:8,desc:"Earth government announces tax incentives. All Earth companies +5% for 8 turns."},
  Mars:{id:"supply_glut",n:"Lithium Supply Glut",ico:"⛏️",prob:.04,priceHit:-.20,dur:12,desc:"Lithium oversupply crashes mining prices 20%. Recovery takes 12 turns. Buy the dip."},
  Venus:{id:"acid_cloud",n:"Acid Cloud Event",ico:"☁️",prob:.05,prodHit:-.15,dur:8,desc:"Sulphuric acid cloud damages solar arrays. Production -15% for 8 turns then full recovery."},
  Jupiter:{id:"great_storm",n:"Great Jupiter Storm",ico:"⚡",prob:.01,prodHit:-.30,dur:20,recovery:true,desc:"Catastrophic storm. Production -30%. MSC activates. Rebuilds in 15-20 turns then booms."},
  Saturn:{id:"ring_destab",n:"Ring Destabilisation",ico:"🪐",prob:.03,priceBoost:.40,dur:5,desc:"Ice export halted 5 turns. Ryzolith shortage pushes prices +40%."},
  Mercury:{id:"solar_flare",n:"Solar Flare Event",ico:"☀️",prob:.04,prodBoost:.50,dur:3,desc:"Maximum solar output. Production +50% for 3 turns. Best short-term boost in game."},
  Uranus:{id:"season_change",n:"Season Change",ico:"🔵",prob:.002,rebalance:true,dur:30,desc:"42-year season boundary causes massive price rebalancing across all Uranus companies."},
  Neptune:{id:"breakthrough",n:"Research Breakthrough",ico:"🔬",prob:.03,spike:true,dur:2,desc:"Neptune research succeeds. One random company stock doubles or triples. Spectacular."},
};
...
export default function PlanetsSim(){
  ...
  if(!S.current){
    S.current={
      turn:1,
      wallets:{Earth:500000,Mars:0,Venus:0,Jupiter:0,Saturn:0,Mercury:0,Uranus:0,Neptune:0},
  ...
  const advance=useCallback(()=>{
    ...
    ALL_PLANETS.forEach(pd=>{
      ...
      // Price each company
      p.cos=p.cos.map(c=>{
        const stormMult=p.stormActive?-.30:0;
        // EPS-ANCHORED: noise around fair value, NOT compounding on current price
        const eps=c.ip/(c.pe||15); // EPS is FIXED from starting price — never changes
        const PEB2={Technology:{mn:15,mx:35},Banking:{mn:8,mx:15},Mining:{mn:6,mx:12},Energy:{mn:8,mx:20},Agriculture:{mn:10,mx:18},Healthcare:{mn:12,mx:35},Manufacturing:{mn:10,mx:25},Utilities:{mn:12,mx:18},"Real Estate":{mn:8,mx:16},Telecom:{mn:10,mx:16},Retail:{mn:10,mx:18}};
        const bnd2=PEB2[c.s]||{mn:10,mx:30};
        const macroMod=cl(p.gdp/100*.15*c.b,-.05,.05);
        const noise=(Math.random()-.5)*.06*c.b;
        const sentiment=stormMult;
        // Fair value = EPS × midpoint P/E, then noise around that
        const midPE=(bnd2.mn+bnd2.mx)/2;
        const fairValue=eps*midPE;
        // Price gravitates toward fair value with noise
        const pull=(fairValue-c.price)/fairValue*.02;
        const totalMove=1+noise+macroMod+sentiment+pull;
        let np=cl(Math.round(c.price*totalMove*100)/100,eps*bnd2.mn,eps*bnd2.mx);
        // Hard absolute ceiling: never more than 3× starting price
        np=cl(np,c.ip*.20,c.ip*3);
        np=Math.max(.10,Math.round(np*100)/100);
        // Post-storm recovery bounce
        if(pd.stormRisk&&p.stormTurn>0&&!p.stormActive&&s.turn-p.stormTurn<10)np=Math.round(np*1.015*100)/100;
    ...
    if(typeof PLANET_EVENTS!=="undefined"){
      Object.entries(PLANET_EVENTS).forEach(([planetName,ev])=>{
        const pd=ALL_PLANETS.find(p=>p.name===planetName);if(!pd)return;
        const ps=s.planets[pd.id];if(!ps)return;
        if(Math.random()<ev.prob&&!ps.activeEvent){
          ps.activeEvent={...ev,tl:ev.dur,dur:ev.dur};
          nn.push({t:s.turn,ico:ev.ico,planet:planetName,msg:"🚨 "+planetName.toUpperCase()+": "+ev.n+" — "+ev.desc,bad:!ev.prodBoost&&!ev.priceBoost&&!ev.spike,mine:false});
          if(ps.cos){
            if(ev.prodHit)ps.cos=ps.cos.map(c=>({...c,stormHit:ev.prodHit}));
            if(ev.priceBoost)ps.cos=ps.cos.map(c=>({...c,price:Math.max(.10,Math.round(c.price*(1+ev.priceBoost)*100)/100)}));
            if(ev.prodBoost)ps.cos=ps.cos.map(c=>({...c,price:Math.max(.10,Math.round(c.price*(1+ev.prodBoost)*100)/100)}));
            if(ev.spike){const tgt=ps.cos[Math.floor(Math.random()*ps.cos.length)];const mult=1.5+Math.random()*1.5;tgt.price=Math.min(tgt.ip*3,Math.round(tgt.price*mult*100)/100);nn.push({t:s.turn,ico:"💎",planet:planetName,msg:"BREAKTHROUGH: "+tgt.n+" price surge! Research succeeded.",bad:false,mine:false});}
          }
        }
        if(ps.activeEvent){
          ps.activeEvent.tl--;
          if(ps.activeEvent.tl<=0){
            nn.push({t:s.turn,ico:ps.activeEvent.ico,planet:planetName,msg:planetName+": "+ps.activeEvent.n+" resolved. Economy stabilising.",bad:false,mine:false});
            if(ps.activeEvent.recovery&&ps.cos)ps.cos=ps.cos.map(c=>({...c,stormHit:0,price:Math.min(c.ip*3,Math.round(c.price*1.04*100)/100)}));
            ps.activeEvent=null;
          }
        }
      });
    }
    s.newsLog=[...nn,...s.newsLog].slice(0,80);
    refresh();
  },[refresh]);
  ...
  const doTrade=(planetId,co,mode,qty)=>{
    ...
    // Pause auto-sim during trade for clarity
    setAuto(false);
    if(isBuy){
      ...
      s.newsLog.unshift({t:s.turn,ico:"📈",planet:pd.name,msg:"BOUGHT "+q.toLocaleString()+" "+co.t+" @ "+fm(co.price)+" "+pd.currency+" · Total: "+fm(cost)+" · "+pd.name+" wallet: "+fm(s.wallets[pd.name]),bad:false});
      toast_("✅ Bought "+q.toLocaleString()+" "+co.n+" @ "+fm(co.price)+" "+pd.currency+" · Cost: "+fm(cost));
    }else{
      ...
      const net=proc-cgt;
      s.wallets[pd.name]=Math.round((walletBal+net)*100)/100;
      s.sh[key]=held-q;if(!s.sh[key])delete s.sh[key];
      s.newsLog.unshift({t:s.turn,ico:"📉",planet:pd.name,msg:"SOLD "+q.toLocaleString()+" "+co.t+" @ "+fm(co.price)+" · Proceeds: "+fm(proc)+" · CGT: "+fm(cgt)+" · Net: "+fm(net),bad:false});
      toast_("✅ Sold "+q.toLocaleString()+" "+co.n+" · Net: "+fm(net)+" (CGT: "+fm(cgt)+")");
    }
  ...
  return <div style={{maxWidth:430,margin:"0 auto",background:"#F0F4F0",minHeight:"100vh",fontFamily:"system-ui,sans-serif"}}>
    ...
    <div style={{background:"linear-gradient(135deg,#060B16,#0D1E35)",padding:"14px 16px",color:"#fff"}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:10}}>
        {ALL_PLANETS.map(pd=><div key={pd.id} onClick={()=>{setSelPlanet(pd.name);setTab("planet");}} style={{flexShrink:0,background:selPlanet===pd.name?"rgba(255,255,255,.2)":"rgba(255,255,255,.08)",borderRadius:8,padding:"6px 8px",textAlign:"center",border:"1px solid "+(selPlanet===pd.name?"rgba(255,255,255,.4)":"rgba(255,255,255,.08)"),cursor:"pointer",minWidth:64}}>
      ...
      <div style={{display:"flex",gap:6}}>
        ...
        {[.5,1,2,5].map(s2=><button key={s2} onClick={()=>setSpeed(s2)} style={{padding:"5px 8px",borderRadius:7,border:"1.5px solid "+(speed===s2?"#fff":"rgba(255,255,255,.25)"),background:speed===s2?"rgba(255,255,255,.2)":"transparent",color:"#fff",fontWeight:700,fontSize:9,cursor:"pointer"}}>{s2<1?".5s":s2+"s"}</button>)}
      </div>
    </div>
    {/* Tabs */}
    ...
    <div style={{padding:14,display:"flex",flexDirection:"column",gap:12}}>
      ...
      {tab==="transfer"&&<>
        <div style={{background:"#fff",borderRadius:13,padding:13,border:"1px solid #e8ebe8"}}>
          ...
          {(()=>{
  const ROUTES=[
    ["Earth","Mars"],["Earth","Venus"],["Earth","Jupiter"],["Earth","Saturn"],["Earth","Mercury"],["Earth","Uranus"],["Earth","Neptune"],
    ["Mars","Earth"],["Venus","Earth"],["Jupiter","Earth"],["Saturn","Earth"],["Mercury","Earth"],["Uranus","Earth"],["Neptune","Earth"],
    ["Mars","Jupiter"],["Jupiter","Mars"],["Saturn","Neptune"],["Mercury","Venus"],
  ];
  return ROUTES.filter(([from])=>(d.wallets?.[from]||0)>0).map(([from,to])=>{
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:8}}>
        {[10,25,50,100].map(pct=>{
          ...
          return <button key={pct} onClick={()=>{
            if(sendAmt<=0){toast_("Insufficient balance",false);return;}
            setConfirmTransfer({from,to,pct,sendAmt,feeAmt,recvAmt,fromCur:fpd.currency,toCur:tpd.currency,fromIco:fpd.ico,toIco:tpd.ico});
    ...
    {confirmTransfer&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#fff",borderRadius:18,padding:22,width:"100%",maxWidth:400}}>
        <div style={{fontSize:11,color:"#999",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Confirm Transfer</div>
        <div style={{fontSize:18,fontWeight:800,color:DK,marginBottom:16}}>{confirmTransfer.fromIco} {confirmTransfer.from} → {confirmTransfer.toIco} {confirmTransfer.to}</div>
        <div style={{background:"#f8fbf8",borderRadius:12,padding:14,marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid #f0f0f0"}}><span style={{fontSize:13,color:"#666"}}>You send</span><span style={{fontSize:13,fontWeight:700,color:R,fontFamily:"monospace"}}>−{fm(confirmTransfer.sendAmt)} {confirmTransfer.fromCur}</span></div>
          <div style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid #f0f0f0"}}><span style={{fontSize:13,color:"#666"}}>Transfer fee (0.5%)</span><span style={{fontSize:13,fontWeight:600,color:"#888",fontFamily:"monospace"}}>−{fm(confirmTransfer.feeAmt)} {confirmTransfer.fromCur}</span></div>
          <div style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid #f0f0f0"}}><span style={{fontSize:13,color:"#666"}}>Destination tax</span><span style={{fontSize:12,color:"#aaa"}}>Applied silently at destination era rate</span></div>
          <div style={{display:"flex",justifyContent:"space-between",padding:"7px 0"}}><span style={{fontSize:14,fontWeight:700,color:DK}}>You receive</span><span style={{fontSize:14,fontWeight:800,color:G,fontFamily:"monospace"}}>+{fm(confirmTransfer.recvAmt)} {confirmTransfer.toCur}</span></div>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>setConfirmTransfer(null)} style={{flex:1,background:"#f0f0f0",color:"#666",border:"none",borderRadius:11,padding:"13px 0",fontWeight:700,fontSize:14,cursor:"pointer"}}>Cancel</button>
          <button onClick={()=>{
            const s=S.current,ct=confirmTransfer;
            if(ct.sendAmt>(s.wallets?.[ct.from]||0)){toast_("Insufficient balance",false);setConfirmTransfer(null);return;}
            s.wallets[ct.from]=Math.round(((s.wallets[ct.from]||0)-ct.sendAmt)*100)/100;
            s.wallets[ct.to]=Math.round(((s.wallets[ct.to]||0)+ct.recvAmt)*100)/100;
            s.transferLog=[{t:s.turn,from:ct.from,to:ct.to,sentAmt:ct.sendAmt,sentCur:ct.fromCur,recvAmt:ct.recvAmt,recvCur:ct.toCur,fee:ct.feeAmt},...(s.transferLog||[])].slice(0,30);
            s.newsLog.unshift({t:s.turn,ico:"💱",planet:ct.from,msg:ct.pct+"% transferred: "+fm(ct.sendAmt)+" "+ct.fromCur+" → "+fm(ct.recvAmt)+" "+ct.toCur+". Fee: "+fm(ct.feeAmt),bad:false});
            toast_("✅ Transferred "+fm(ct.sendAmt)+" "+ct.fromCur+" → "+fm(ct.recvAmt)+" "+ct.toCur);
            setConfirmTransfer(null);refresh();
          }} style={{flex:2,background:G,color:"#fff",border:"none",borderRadius:11,padding:"13px 0",fontWeight:800,fontSize:14,cursor:"pointer"}}>✅ Confirm Transfer</button>
        </div>
      </div>
    </div>}
