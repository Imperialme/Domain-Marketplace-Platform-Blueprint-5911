...
export default function App(){
  ...
  const advance=useCallback(()=>{
    ...
    // GSF — never reduce deposit, just pay interest every turn
    if(s.gsfDep>0){
    ...
    // Quarterly dividends (every 30 turns = quarterly, div% is annual so pay /4)
    if(s.turn%30===0){
      let div=0;
      Object.entries(s.sh).forEach(([t,n])=>{const c=s.cos.find(x=>x.t===t);if(c&&n>0)div+=c.price*(c.div/100/4)*n;});
      if(div>0){s.cash=Math.round((s.cash+div)*100)/100;nn.push({id:Math.random(),t:s.turn,ico:"💰",ti:"Quarterly Dividends",bo:fm(div)+" credited. Annual yield ÷ 4 per quarter.",g:true});}
      // Bond coupons quarterly
      let coup=0;
      Object.entries(s.bh).forEach(([id,q])=>{const b=s.bonds.find(x=>x.id===id);if(b&&q>0)coup+=b.fv*(b.cou/100/4)*q;});
      if(coup>0){s.cash=Math.round((s.cash+coup)*100)/100;nn.push({id:Math.random(),t:s.turn,ico:"📋",ti:"Bond Coupons",bo:fm(coup)+" credited quarterly.",g:true});}
    }
  ...
  const S_GSF=<div style={{padding:14,display:"flex",flexDirection:"column",gap:12}}>
    ...
    <div style={{background:"#E3F2FD",borderRadius:13,padding:14,border:"1px solid #BBDEFB"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        ...
        <SB l="Per Turn" v={fm((d.gsfDep||0)*((d.gsf||12.48)/100/12))} c={G}/>
      </div>
      <div style={{background:"#fff",borderRadius:8,padding:"10px 12px",marginBottom:12,fontSize:12,color:"#666",lineHeight:1.6}}>
        ...
        Return calc: {fm(d.gsfDep||0)} × {(d.gsf||12.48).toFixed(2)}% ÷ 12 = {fm((d.gsfDep||0)*((d.gsf||12.48)/100/12))}/turn<br/>
  ...
  return <div style={{maxWidth:430,margin:"0 auto",background:"#F0F4F0",minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:"system-ui,-apple-system,sans-serif"}}>
    ...
    <div style={{background:"#fff",padding:"10px 15px",borderBottom:"1px solid #e8ebe8",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        <div><div style={{fontSize:15,fontWeight:800,color:DK,lineHeight:1}}>Galactic Raider</div><div style={{fontSize:9,color:"#bbb"}}>Capital Exchange · v7</div></div>
      </div>
    ...
    {donM&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.65)",zIndex:300,display:"flex",alignItems:"flex-end"}} onClick={e=>e.target===e.currentTarget&&setDonM(null)}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:20,width:"100%"}}>
        ...
        <button onClick={()=>{if(!donAmt||donAmt<1||donAmt>d.cash){toast_("Select amount",false);return;}const s=S.current;s.cash=Math.round((s.cash-donAmt)*100)/100;s.dons=(s.dons||0)+1;s.taxRed=Math.min(0.25,(s.taxRed||0)+0.05);s.news.unshift({id:Math.random(),t:s.turn,ico:"❤️",ti:"Donation: "+donM,bo:fm(donAmt)+" donated. CGT now "+(20*(1-s.taxRed)).toFixed(0)+"% on profit.",g:true});toast_("Donated "+fm(donAmt)+" ❤️");setDonM(null);setDonAmt(null);refresh();}} style={{width:"100%",background:"#880E4F",color:"#fff",border:"none",borderRadius:12,padding:14,fontWeight:800,fontSize:14,cursor:"pointer"}}>❤️ Confirm{donAmt?" — "+fm(donAmt):""}</button>
      </div>
    </div>}
    {/* GSF deposit modal */}
    {gsfM&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.65)",zIndex:300,display:"flex",alignItems:"flex-end"}} onClick={e=>e.target===e.currentTarget&&setGsfM(false)}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:20,width:"100%"}}>
        ...
        <div style={{fontSize:12,color:"#888",marginBottom:12}}>Rate: {(d.gsf||12.48).toFixed(2)}%/yr · Cash: {fm(d.cash)} · Per turn after: {fm(((d.gsfDep||0)+(gsfAmt||0))*((d.gsf||12.48)/100/12))}</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:7,marginBottom:12}}>
        ...
        {gsfAmt&&gsfAmt>0&&<div style={{background:"#E3F2FD",borderRadius:10,padding:12,marginBottom:12}}><Row k="Deposit" v={fm(gsfAmt)} vc={BL} b/><Row k="New per-turn return" v={fm(((d.gsfDep||0)+gsfAmt)*((d.gsf||12.48)/100/12))} vc={G}/></div>}
        <button onClick={()=>{if(!gsfAmt||gsfAmt<1||gsfAmt>d.cash){toast_("Select amount",false);return;}const s=S.current;s.cash=Math.round((s.cash-gsfAmt)*100)/100;s.gsfDep=(s.gsfDep||0)+gsfAmt;toast_("Deposited "+fm(gsfAmt)+" → "+fm(s.gsfDep*(s.gsf/100/12))+"/turn");setGsfM(false);setGsfAmt(null);refresh();}} style={{width:"100%",background:BL,color:"#fff",border:"none",borderRadius:12,padding:14,fontWeight:800,fontSize:14,cursor:"pointer"}}>🏛️ Confirm{gsfAmt?" — "+fm(gsfAmt):""}</button>
      </div>
    </div>}
    {/* Quiz */}
