...
const LOAN_TIERS=[
  ...
  {tier:4,max:1000000,rate:.18,requires:"Tier 3 fully repaid",label:"Business Loan"},
  {tier:5,max:5000000,rate:.22,requires:"Tier 4 fully repaid + $500K NW",label:"Corporate Loan"},
  {tier:6,max:25000000,rate:.25,requires:"Tier 5 repaid + $5M NW",label:"Premium Loan"},
];
...
export default function WalletsSim(){
  ...
  const openFoundation=(name)=>{
    const s=S.current;
    const cost=50000;// Foundation setup fee
  ...
  return <div style={{maxWidth:430,margin:"0 auto",background:"#F0F4F0",minHeight:"100vh",fontFamily:"system-ui,sans-serif"}}>
    ...
    <div style={{padding:14,display:"flex",flexDirection:"column",gap:12}}>
      ...
      {tab==="wallets"&&<>
        ...
        {[
          ...
          {w.key==="savingsWallet"&&!d.foundationOpen&&<button onClick={()=>setFoundationM(true)} style={{width:"100%",marginTop:8,background:AU,color:"#fff",border:"none",borderRadius:8,padding:"9px 0",fontWeight:700,fontSize:12,cursor:"pointer"}}>🏛️ Open Foundation to Protect Savings ($50K fee)</button>}
      ...
      {tab==="autobuy"&&<>
        ...
        <div style={{background:"#fff",borderRadius:14,padding:14,border:"1px solid #e8ebe8"}}>
          ...
          <div style={{marginBottom:10}}>
            <div style={{fontSize:11,color:"#999",marginBottom:5}}>Select Company — tap to favourite ⭐</div>
            <input type="text" placeholder="Search ticker (SLKT, MDCR...)" value={newAutoTicker} onChange={e=>setNewAutoTicker(e.target.value.toUpperCase())} style={{width:"100%",padding:"10px 12px",borderRadius:9,border:"1.5px solid #e0e0e0",fontSize:12,outline:"none",marginBottom:8,fontFamily:"monospace",background:"#fafafa"}}/>
            <div style={{fontSize:11,color:"#888",marginBottom:6}}>Quick select (tap ticker):</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
              {["SLKT","MDCR","TNPT","FRMN","UTLS","TLCM","RLST","EMTS","AGRO","NRDX"].map(t=><button key={t} onClick={()=>setNewAutoTicker(newAutoTicker===t?"":t)} style={{padding:"6px 10px",borderRadius:8,border:"1.5px solid "+(newAutoTicker===t?G:"#e0e0e0"),background:newAutoTicker===t?"#E8F5E9":"#fafafa",color:newAutoTicker===t?G:"#555",fontWeight:700,fontSize:10,cursor:"pointer"}}>{t}</button>)}
            </div>
      ...
      {tab==="loans"&&<>
        ...
        {LOAN_TIERS.map((tier,i)=>{
          ...
          return <div key={i} style={{background:"#fff",borderRadius:13,padding:13,border:"2px solid "+(isActive?"#B71C1C":isRepaid?"#A5D6A7":isNext?"#1565C0":"#e0e0e0"),opacity:isLocked?.5:1}}>
            ...
            {isActive&&<>
              <div style={{marginBottom:6,fontSize:11,color:R,fontWeight:600}}>⚠️ Active loan</div>
              <div style={{display:"flex",gap:5,marginBottom:8}}>
                {[{l:"25%",amt:Math.round((d.activeLoan?.outstanding||0)*.25*100)/100},{l:"50%",amt:Math.round((d.activeLoan?.outstanding||0)*.50*100)/100},{l:"All",amt:d.activeLoan?.outstanding||0}].map(b=><button key={b.l} onClick={()=>repayLoan(b.amt)} disabled={b.amt>(d.tradingWallet||0)} style={{flex:1,background:b.amt>(d.tradingWallet||0)?"#f0f0f0":R,color:b.amt>(d.tradingWallet||0)?"#aaa":"#fff",border:"none",borderRadius:7,padding:"8px 0",fontWeight:700,fontSize:11,cursor:b.amt>(d.tradingWallet||0)?"not-allowed":"pointer"}}>Repay {b.l}</button>)}
              </div>
            </>}
    ...
    {foundationM&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.65)",zIndex:300,display:"flex",alignItems:"flex-end"}} onClick={e=>e.target===e.currentTarget&&setFoundationM(false)}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:20,width:"100%"}}>
        ...
        <button onClick={()=>openFoundation("My Foundation")} disabled={(d.cashWallet||0)<50000} style={{width:"100%",background:(d.cashWallet||0)>=10000?PU:"#e0e0e0",color:"#fff",border:"none",borderRadius:12,padding:14,fontWeight:800,fontSize:14,cursor:(d.cashWallet||0)>=10000?"pointer":"not-allowed"}}>🏛️ Open Foundation — $50K fee{(d.cashWallet||0)<50000?" (need $50K in Cash)":""}</button>
      </div>
    </div>}
