    ...
    <div style={{padding:14,display:"flex",flexDirection:"column",gap:10}}>
      ...
      {tab==="co"&&(()=>{
        ...
        return <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          {/* Analyst opinions — all firms */}
          <div style={{background:"#1E293B",borderRadius:12,padding:13,border:"1px solid #334155"}}>
            <div style={{fontSize:13,fontWeight:700,color:"#F8FAFC",marginBottom:10}}>Analyst Opinions — {c.analysts.length} firms covering {c.t}</div>
            {c.analysts.map((a,i)=><div key={i} style={{padding:"10px 0",borderBottom:i<c.analysts.length-1?"1px solid rgba(255,255,255,.06)":"none"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                <div style={{display:"flex",alignItems:"center",gap:7}}>
                  <span style={{fontSize:12,fontWeight:700,color:"#F8FAFC"}}>{a.firm}</span>
                  <span style={{background:a.rating==="STRONG BUY"?"#14532D":a.rating==="BUY"?"#166534":a.rating==="HOLD"?"#78350F":"#7F1D1D",color:a.rating==="STRONG BUY"||a.rating==="BUY"?"#BBF7D0":a.rating==="HOLD"?"#FDE68A":"#FCA5A5",padding:"2px 8px",borderRadius:12,fontSize:9,fontWeight:700}}>{a.rating}</span>
                </div>
                <span style={{fontSize:12,fontWeight:700,fontFamily:"monospace",color:a.target>c.price?"#86EFAC":"#FCA5A5"}}>Tgt {fm(a.target)}</span>
              </div>
              <div style={{fontSize:11,color:"#94A3B8",lineHeight:1.5}}>{a.note}</div>
            </div>)}
          </div>
          ...
          {(()=>{
            const TOTAL_SHARES={SLKT:1200000000,MRDB:800000000,FRMN:600000000,TNPT:900000000,MDCR:400000000,UTLS:300000000,TLCM:550000000,RETL:650000000,RLST:250000000,EMTS:180000000,AGRO:500000000};
            const total=TOTAL_SHARES[c.t]||500000000;
            const ownPct=held>0?Math.round(held/total*10000)/100:0;
            return <div style={{background:"#1E293B",borderRadius:12,padding:13,border:"1px solid #334155"}}>
              <div style={{fontSize:13,fontWeight:700,color:"#F8FAFC",marginBottom:10}}>Share Structure</div>
              {[["Total Shares Outstanding",(total/1e6).toFixed(0)+"M shares"],["Your Holdings",held>0?held.toLocaleString()+" shares":"None"],["Your Ownership",held>0?ownPct.toFixed(3)+"%":"0%"],["Board Access",ownPct>=50?"✅ Majority Control":ownPct>=25?"✅ Significant Control":ownPct>=10?"✅ Board Seat":"❌ None (need 10%)"]].map(([k,v])=><div key={k} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,.06)"}}><span style={{fontSize:11,color:"#94A3B8"}}>{k}</span><span style={{fontSize:11,fontWeight:700,color:k==="Board Access"&&ownPct>=10?"#86EFAC":"#F8FAFC"}}>{v}</span></div>)}
              {held>0&&<div style={{marginTop:8,height:5,background:"rgba(255,255,255,.1)",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:Math.min(100,ownPct*2)+"%",background:"linear-gradient(90deg,#16A34A,#7C3AED)",borderRadius:3}}/></div>}
            </div>;
          })()}
